...
import { Search, Info, ChevronDown, Zap, Loader2 } from 'lucide-react';
...
export function InputPanel() {
  const {
    selectedSubstance, mass, congestion, position, cloudType,
    isCalculating, result,
    setSubstance, setMass, setCongestion, setPosition, setCloudType,
    runCalculation,
  } = useStore();
  ...
  return (
    <aside className="w-[300px] min-w-[280px] flex flex-col border-r border-slate-800 bg-slate-900/30 overflow-y-auto">
      ...
      <div className="px-5 pt-5 pb-4 border-b border-slate-800">
        ...
        <p className="text-slate-500 mt-1" style={{ fontSize: 13 }}>Введите параметры для расчёта</p>
      </div>
      ...
      <div className="flex flex-col gap-5 p-5">
        ...
        <div>
          ...
          <div className="relative">
            ...
            {dropdownOpen && (
              <div className="absolute z-50 w-full mt-1 rounded-lg border border-slate-700 bg-slate-900 shadow-2xl shadow-black/60">
                ...
                <div className="max-h-64 overflow-y-auto py-1">
                  {grouped.map(group => (
                    <div key={group.cls}>
                      ...
                      {group.items.map(s => (
                        ...
                        >
                          <span className={`shrink-0 px-1.5 py-0.5 rounded border font-mono ${CLASS_COLORS[s.class]}`} style={{ fontSize: 11 }}>
                            К{s.class}
                          </span>
                          <span className="text-slate-300" style={{ fontSize: 13 }}>{s.name}</span>
                          {s.formula && (
                            <span className="text-slate-600 font-mono ml-auto shrink-0" style={{ fontSize: 12 }}>{s.formula}</span>
                          )}
                        </button>
                      ))}
                    </div>
                  ...
                  {grouped.length === 0 && (
                    <p className="px-3 py-4 text-slate-600 text-center" style={{ fontSize: 13 }}>Ничего не найдено</p>
                  )}
                </div>
              </div>
            )}
        ...
        <div>
          <label className="block font-medium text-slate-400 mb-2" style={{ fontSize: 13 }}>
            Масса горючего вещества M<sub>T</sub>
          </label>
          <div className="flex items-center gap-2">
            <input
              ...
              style={{ fontSize: 14 }}
              className="flex-1 px-3 py-2.5 rounded-lg bg-slate-800/60 border border-slate-700 focus:border-blue-500 focus:outline-none text-slate-200 transition-colors"
            />
            <span className="text-slate-500 font-mono shrink-0" style={{ fontSize: 14 }}>кг</span>
          </div>
          <p className="text-slate-600 mt-1" style={{ fontSize: 12 }}>от 1 до 1 000 000 кг</p>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {([['gas', 'Газо-/паровоздушная', 'σ = 7'], ['dust', 'Пылевоздушная', 'σ = 4']] as const).map(([val, label, sub]) => (
              <button
                key={val}
                onClick={() => setCloudType(val)}
              ...
              >
                <div className="font-medium" style={{ fontSize: 13 }}>{label}</div>
                <div className="text-slate-600 mt-0.5" style={{ fontSize: 12 }}>{sub}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {([['ground', 'На поверхности земли', 'E × 2'], ['air', 'В воздухе', 'E × 1']] as const).map(([val, label, hint]) => (
              <button
                key={val}
              ...
              >
                <div className="font-medium" style={{ fontSize: 13 }}>{label}</div>
                <div className="text-slate-600 mt-0.5" style={{ fontSize: 12 }}>{hint}</div>
              </button>
            ))}
          </div>
        </div>
